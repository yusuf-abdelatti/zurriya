  ...
  return (
    ...
    >
      <nav className={`max-w-content mx-auto px-6 h-16 flex items-center justify-between gap-6 ${isAr ? 'rtl:flex-row-reverse' : ''}`}>
        ...
        {/* Right side */}
        <div className={`hidden md:flex items-center gap-3 ${isAr ? 'rtl:flex-row-reverse' : ''}`}>
          <Link
            href={`/${otherLocale}`}
        ...
        {/* Mobile controls */}
        <div className={`md:hidden flex items-center gap-2 ${isAr ? 'rtl:flex-row-reverse' : ''}`}>
      ...
      {/* Mobile drawer */}
      {mobileOpen && (
        <div className={`md:hidden bg-paper border-t border-border px-6 py-6 ${isAr ? 'rtl:text-right' : ''}`}>
